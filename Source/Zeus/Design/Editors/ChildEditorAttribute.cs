	...
	public class ChildEditorAttribute : AbstractEditorAttribute
	{
		...
		public override bool UpdateItem(IEditableObject item, Control editor)
		{
			...
			ItemUtility.FindInParents<ItemEditView>(editor.Parent).Saved += ((sender, args) =>
			{
                ...
                if (!((ContentItem)itemEditor.CurrentItem).IsEmpty() || (((ContentItem)itemEditor.CurrentItem) is File && ((BaseFileUpload)editor.Controls[0].Controls[1]).HasNewOrChangedFile))
                {
