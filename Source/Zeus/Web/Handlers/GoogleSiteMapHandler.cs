...
namespace Zeus.Web.Handlers
{
	...
	public class GoogleSiteMapHandler : IHttpHandler
	{
		...
		private static string GetSiteMap(HttpContext context)
		{
			...
			using (var writer = XmlWriter.Create(stringWriter, settings))
			{
				...
				foreach (var item in list)
				{
					...
					writer.WriteElementString("lastmod", item.Published.GetValueOrDefault(item.Updated).ToString("yyyy-MM-dd")); // Google doesn't like IS0 8601/W3C 
			...
			context.Cache.Add("GoogleSitemap", builder.ToString(), null, DateTime.Today.AddDays(3),
